...
| 8 | `supabase/schema-auth-link.sql` | Makes `merchants.wallet_address` optional and adds `auth_user_id` (linked to Supabase Auth) — a merchant account can now start from Google sign-in with no wallet, and link one later from inside the dashboard via `/api/train/link-wallet` onto the same account. |
| 9 | `supabase/schema-tutor-sessions.sql` | Adds `session_id` to `tutor_turns` so the "Tutor Clerk" panel can rehearse a whole back-and-forth conversation, not just one question — mirrors real multi-turn tickets (`conversation.mjs`). |
| 10 | `supabase/schema-principles.sql` | `reply_principles` table — replaces `exemplar_replies` as what shapes future drafts. Every human correction/resolution is distilled into a generalized principle (the reasoning, not the wording), so Clerk never parrots identical phrasing to different customers. `exemplar_replies` is left in place but deprecated — no longer written to or read from. |
...
| `clerk-neo.html` | **Live onboarding + merchant dashboard.** Connect Wallet (free `registerMerchant`, gas sponsored automatically) or Continue with Google — either becomes the same merchant account; a wallet can be linked later from inside the dashboard. Dashboard reads tickets and reputation directly from the deployed contract (no simulated data) and has a "Train Clerk" tab (FAQ upload + multi-turn tutoring) and a "Clerk's resolved tickets" tab (exact transcripts of every self-resolved ticket), both backed by `/api/train/*`. | Set `window.CLERK_CONTRACT`, `window.SUPABASE_URL`, `window.SUPABASE_ANON_KEY`; serve over http (see below) |
...
`clerk-neo.html`'s "Live queue" tab and `portal.html` show what's actually knowable from the chain alone: ticket hash, status, confidence, payout, reopen action. They do NOT show customer names, message text, or Clerk's drafted reply — that content lives off-chain in Supabase by design (see `schema.sql`). Merchant-authenticated access to that off-chain layer exists via the Train Clerk panel (`/api/train/*`, gated by a wallet-signature or Google session — see `schema-auth-link.sql`), which now includes a "Clerk's resolved tickets" tab (`/api/train/resolved-tickets`, `schema-principles.sql`/`schema-tutor-sessions.sql` era) showing the exact transcript of every ticket Clerk resolved on its own.
