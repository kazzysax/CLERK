...
| `clerk-neo.html` | **Live onboarding + merchant dashboard.** Connect Wallet (free `registerMerchant`, gas sponsored automatically) or Continue with Google — either becomes the same merchant account; a wallet can be linked later from inside the dashboard. Dashboard reads tickets and reputation directly from the deployed contract (no simulated data) and has a "Train Clerk" tab (FAQ upload + tutoring) backed by `/api/train/*`. | Set `window.CLERK_CONTRACT`, `window.SUPABASE_URL`, `window.SUPABASE_ANON_KEY`; serve over http (see below) |
